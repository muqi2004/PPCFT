...
def schedule_dag(dag, computation_matrix, communication_matrix, proc_schedules=None, time_offset=0, relabel_nodes=True, **kwargs):
    ...
    for task in dag.nodes():
        _self.optimistic_cost_table[task] = [0] * num_processors  # 每个任务对应一个全零列表
    ...
    for node in dag.nodes():
        if _self.optimistic_cost_table[node]:  # Ensure the table is populated
            dag.nodes[node]['ranku'] = sum(_self.optimistic_cost_table[node]) / len(_self.communication_matrix)      
            ...
            for succnode in dag.successors(node):
                ...
                for preproc in range(len(_self.communication_matrix)):
                    if preproc != proc:
                        ...
                        val = _self.optimistic_cost_table[succnode][preproc]+dag[node][succnode]['weight']/_self.communication_matrix[preproc][proc]+_self.computation_matrix[succnode][preproc]
                    else: 
