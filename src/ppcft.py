...
def schedule_dag(dag, computation_matrix, communication_matrix, proc_schedules=None, time_offset=0, relabel_nodes=True, **kwargs):
    ...
    _self = {
        ...
        'cost_predict_matrix':{},
        'harmonic_weight':{},
        'historical_cost_predict_matrix':{}
    }
    _self = SimpleNamespace(**_self)
    
    num_processors = len(_self.computation_matrix[0]) if _self.computation_matrix is not None else 0  # 获取处理器数量
    for task in dag.nodes():
        _self.optimistic_cost_table[task] = [0.0] * num_processors  # 每个任务对应一个全零列表
        _self.cost_predict_matrix[task] = [0.0] * num_processors
        _self.harmonic_weight[task] = [0.0]*num_processors
        _self.historical_cost_predict_matrix[task] = [0.0] * num_processors
    ...
    _compute_rankd(_self, dag, **kwargs)
    
    for node in dag.nodes():
        costs = _self.cost_predict_matrix[node]
        for proc in range(len(_self.communication_matrix)):
            _self.harmonic_weight[node][proc] = (1/costs[proc]) / sum(1.0 / cost for cost in costs)
        
    for node in dag.nodes():  
        costs = _self.cost_predict_matrix[node]
        if any(cost == 0 for cost in costs):  # 避免除以零
            dag.nodes[node]['ranku'] = 0.0
        else:
            dag.nodes[node]['ranku'] = len(costs) / sum(1.0 / cost for cost in costs)
        print(f'{node} : {dag.nodes[node]['ranku']:.2f}')        
    print(_self.historical_cost_predict_matrix)
    ...
    pre_processors = {}
    for node in dag.nodes():
        ipcm_values = [_self.cost_predict_matrix[node][i]+_self.historical_cost_predict_matrix[node][i] for i in range(len(_self.communication_matrix))]
        pre_processors[node] = np.argmin(ipcm_values) 
    print(pre_processors)
    for i, node in enumerate(sorted_nodes):
        for j in range(i):
            prev_node = sorted_nodes[j]
            if pre_processors[node] == pre_processors[prev_node]:
                if (_self.cost_predict_matrix[prev_node][proc] >= _self.cost_predict_matrix[node][proc] and
                    _self.cost_predict_matrix[prev_node][proc] - _self.computation_matrix[node][proc] < _self.cost_predict_matrix[node][proc]):
                    _self.cost_predict_matrix[prev_node][proc] = max(_self.cost_predict_matrix[prev_node][proc] , _self.computation_matrix[prev_node][proc]+_self.cost_predict_matrix[node][proc])
    task_queue = []
    ...
    while task_queue:
        ...
        for proc in range(len(communication_matrix)):
            ...
            if (taskschedule.end+_self.cost_predict_matrix[node][taskschedule.proc]-_self.computation_matrix[node][taskschedule.proc] < minTaskSchedule.end+_self.cost_predict_matrix[node][minTaskSchedule.proc]-_self.computation_matrix[node][minTaskSchedule.proc]):
...
def _compute_ranku(_self, dag, **kwargs):
    ...
    _self.cost_predict_matrix[terminal_node] = _self.computation_matrix[terminal_node]
    ccr=0.0
    for node in dag.nodes():
        x=0
        for succnode in dag.successors(node):
            x=max(dag[node][succnode]['avgweight'],x)
        ccr +=x
    ccr = ccr/ sum([np.mean(i) for i in _self.computation_matrix])
    while visit_queue:
        ...
        alpha = [1.0]*len(_self.computation_matrix)
        for proc in range(len(_self.communication_matrix)):
            _self.cost_predict_matrix[node][proc] = _self.computation_matrix[node][proc]
            max_optimistic_cost = 0 
            for succnode in dag.successors(node):
                optimistic_cost_ = inf
                for preproc in range(len(_self.communication_matrix)):
                    if preproc != proc:
                        val = _self.cost_predict_matrix[succnode][preproc]+dag[node][succnode]['weight']/_self.communication_matrix[preproc][proc]  
                    else: 
                        val = _self.cost_predict_matrix[succnode][proc]
                    if val < optimistic_cost_ :
                        optimistic_cost_ = val
                        alpha [succnode] = max(alpha[succnode], len(list(dag.successors(node))/(len(_self.communication_matrix)*(ccr))))       
                if optimistic_cost_ >max_optimistic_cost:
                    _self.cost_predict_matrix[node][proc] = optimistic_cost_+_self.computation_matrix[node][proc]
...
def _compute_rankd(_self, dag, **kwargs):
    terminal_node = [node for node in dag.nodes() if not any(True for _ in dag.predecessors(node))]
    assert len(terminal_node) == 1, f"Expected a single terminal node, found {len(terminal_node)}"
    terminal_node = terminal_node[0]
    ...
    # Utilize a masked array so that np.mean, etc, calculations ignore the entries that are inf
    comp_matrix_masked = np.ma.masked_where(_self.computation_matrix == inf, _self.computation_matrix)
    nx.set_node_attributes(dag, { terminal_node: np.mean(comp_matrix_masked[terminal_node-_self.numExistingJobs]) }, "rankd")
    visit_queue = deque(dag.successors(terminal_node))
    
    while visit_queue:
        node = visit_queue.pop()
        while _node_can_be_successed(_self, dag, node) is not True:
            try:
                node2 = visit_queue.pop()
            except IndexError:
                raise RuntimeError(f"Node {node} cannot be successed, and there are no other nodes in the queue to successe instead!")
            visit_queue.appendleft(node)
            node = node2
            
        logger.debug(f"Assigning ranku for node: {node}")
        max_successor_ranku = -1
        for procnode in dag.predecessors(node):
            logger.debug(f"\tLooking at successor node: {procnode}")
            logger.debug(f"\tThe edge weight from node {node} to node {procnode} is {dag[procnode][node]['avgweight']}, and the ranku for node {node} is {dag.nodes()[procnode]['rankd']}")
            val = float(dag[procnode][node]['avgweight']) + dag.nodes()[procnode]['rankd']
            if val > max_successor_ranku:
                max_successor_ranku = val
        assert max_successor_ranku >= 0, f"Expected maximum successor ranku to be greater or equal to 0 but was {max_successor_ranku}"
        nx.set_node_attributes(dag, { node: np.mean(comp_matrix_masked[node-_self.numExistingJobs]) + max_successor_ranku }, "rankd")
        ...
        for proc in range(len(_self.communication_matrix)):
            ...
            for procnode in dag.predecessors(node):
                ...
                for preproc in range(len(_self.communication_matrix)):
                    if preproc != proc:
                        val = _self.historical_cost_predict_matrix[procnode][preproc]+dag[procnode][node]['weight']/_self.communication_matrix[preproc][proc]+_self.computation_matrix[procnode][preproc]
                    else: 
                        val = _self.historical_cost_predict_matrix[procnode][proc]+_self.computation_matrix[procnode][preproc]
                    if val < optimistic_cost_ :
                        optimistic_cost_ = val
                if optimistic_cost_ >max_optimistic_cost:
                    _self.historical_cost_predict_matrix[node][proc] = optimistic_cost_
                    max_optimistic_cost = optimistic_cost_
...
def _node_can_be_processed(_self, dag, node):
    ...
    for succnode in dag.successors(node):
        if 'ranku' not in dag.nodes()[succnode]:
            ...
            return False
    return True
def _node_can_be_successed(_self, dag, node):
    """
    Validates that a node is able to be processed in Rank U calculations. Namely, that all of its successors have their Rank U values properly assigned
    Otherwise, errors can occur in processing DAGs of the form
    A
    ...
    """
    for procnode in dag.predecessors(node):
        if 'rankd' not in dag.nodes()[procnode]:
            logger.debug(f"Attempted to compute the Rank U for node {node} but found that it has an unprocessed successor {dag.nodes()[procnode]}. Will try with the next node in the queue")
            return False
    return True
